local aws = import 'aws.libsonnet';
local rescile = import 'rescile/v1/rescile.libsonnet';

rescile.createResource(
  origin='subscription',
  createFrom=rescile.createFromProperty('account', asName='account'),
  resourceType='account',
  relationType='BASELINE_TEMPLATE',
  name='{{- parent -}}-{{ value }}',
  properties={
    class: '{{- class -}}',
    environment: '{{- value | lower -}}',
    //template = "Core" # Only AWS
    regions: ['{{- home -}}', 'London', 'Paris'],
    description: 'The {{ provider | capitalize }} ' + aws.decode.account + ' defines an independently manageable IAM security boundary for the {{ origin_resource.name | upper }} subscription that enables a centralized management and grants authorized access to resources, services and configurations.',
    created: '{{- timestamp -}}',
  },
) + {
  parent: '{{- origin_resource[0].name | regexp(expr="s/^([^-]+-[^-]+-[^-]+)-.*/\\1/") | upper -}}',
  provider: '{{- origin_resource[0].name | regexp(expr="s/^([^-]+)-.*/\\1/") | capitalize -}}',
  class: '{{- origin_resource[0].class | lower -}}',
  timestamp: '{{- now(utc=true) | date(format="%Y-%m-%dT%H:%M:%SZ") -}}',
  home: '{{- origin_resource[0].home | capitalize -}}',
}
